...
function rpxWPsocial (rpxLabel, rpxSummary, rpxLink, rpxLinkText, rpxComment, rpxImageSrc, rpxPostID, rpxElement) {
    ...
    RPXNOW.init(
        {
            appId: '<?php echo get_option(RPX_APP_ID_OPTION); ?>',
            xdReceiver: '<?php echo RPX_PLUGIN_URL; ?>rpx_xdcomm.html'});
    RPXNOW.loadAndRun(['Social'],
        function () {
          var activity = new RPXNOW.Social.Activity( rpxLabel, rpxLinkText, rpxLink);
...
(function() {
 ...
 e.src = 'https://rpxnow.com/load/<?php echo $realm; ?>';
 } else {
 e.src = 'http://widget-cdn.rpxnow.com/load/<?php echo $realm; ?>';
 }
...
<input id="rpxwidget" type="hidden" name="<?php echo RPX_NEW_WIDGET_OPTION; ?>" value='true' />
<li>
...
<input type="hidden" name="rpx_collect"  value="<?php echo $rpx_http_vars['rpx_collect']; ?>" />
<input type="hidden" name="rpx_session"  value="<?php echo $rpx_http_vars['rpx_session']; ?>" />
<input type="hidden" name="rpx_provider" value="<?php echo $rpx_http_vars['rpx_provider']; ?>" />
<input type="hidden" name="redirect_to"  value="<?php echo $rpx_http_vars['redirect_to']; ?>" />
<input type="hidden" name="rpx_url"  value="<?php echo $_GET['rpx_url']; ?>" />
<input type="hidden" name="rpx_displayname"  value="<?php echo $_GET['rpx_displayname']; ?>" />
<input type="hidden" name="rpx_given"  value="<?php echo $_GET['rpx_given']; ?>" />
<input type="hidden" name="rpx_family"  value="<?php echo $_GET['rpx_family']; ?>" />
<input type="hidden" name="rpx_identifier"  value="<?php echo $_GET['rpx_identifier']; ?>" />
<?php
    if ($collect == 'email'){
        ...
        if(strtolower($rpx_http_vars['rpx_provider']) == 'twitter'){
            if(empty($rpx_http_vars['rpx_email'])){
                echo ' <p>This '.$rpx_http_vars['rpx_provider'].' account did not provide an email address. '."\n".'Enter a valid email address to register this account.</p>';
                echo ' <p><input type="text"   name="rpx_email"    value="" id="rpxemail" class="rpxemail" size="30" /></p>';
            }else if(isset($_GET['rpx_email'])){
                if(email_exists(urldecode($_GET['rpx_email']))){
                    ?>
                    <script type="text/javascript">
                        alert('The email address is already in use. Use another email address or login to that account.');
                        window.location = "<?php echo site_url()."/wp-login.php?action=register"; ?>";
                    </script>
                <?php
                }else if(!empty($_GET['rpx_email'])){
                    echo '<p>Entered email address</p>';
                    echo ' <p><input type="text" name="rpx_email" value="'.$_GET['rpx_email'].'" id="rpxemail" class="rpxemail" size="30" /></p>';
                }
            }
        } else {
...
<?php
    ...
    function rpx_open_widget(){
        ...
        $rpx_wp_profile['rpx_username']    = urldecode($_GET['rpx_username']);
        $rpx_wp_profile['rpx_url']         = urldecode($_GET['rpx_url']);
        $rpx_wp_profile['rpx_email']       = urldecode($_GET['rpx_email']);
        $rpx_wp_profile['rpx_displayname'] = urldecode($_GET['rpx_displayname']);
        $rpx_wp_profile['rpx_given']       = urldecode($_GET['rpx_given']);
        $rpx_wp_profile['rpx_family']      = urldecode($_GET['rpx_family']);
        $rpx_wp_profile['rpx_provider']    = urldecode($_GET['rpx_provider']);
        $rpx_wp_profile['rpx_identifier']  = urldecode($_GET['rpx_identifier']);
...
<?php
    ...
    function rpx_buttons($style=RPX_BUTTONS_STYLE_SMALL, $label_text=RPX_LOGIN_PROMPT, $redirect=NULL, $action=null) {
        ...
        $buttons = '<div class="foo" id="janrainEngageEmbed"></div>';
