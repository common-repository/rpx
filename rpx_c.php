...
function rpx_process_user($action) {
	/*Using a switch for visual clarity, this may be cleaned up when conditionals are no longer in flux.*/
	if ( RPX_VERBOSE == 'true' ) {
		error_log('WP-RPX '.$action);
		/*This will enter every RPX action into your php log (usually apache error log).*/
	}
	switch ( $action ) {
		case 'signon':
			rpx_signon_wp_user();
		...
		case 'create':
			...
			break;
		case 'vemail':
			$wpId = rpx_get_vemail_id();
			if ( $wpId ) {
				# a user already exists with this verified email link them to this identifier and log them in
				rpx_update_wp_user();
				rpx_signon_wp_user();
				return true;
			} else {
...
function rpx_get_vemail_id() {
	global $rpx_wp_profile;
	$linkVerifiedEmails = ( get_option(RPX_VEMAIL_OPTION) == 'true' );
	$verifiedEmail = $rpx_wp_profile['rpx_verifiedEmail'];
	if ( $linkVerifiedEmails && $verifiedEmail ) {
		# the user logged in with a verified email and verified email linking is enabled
		if ( $wpId = email_exists( $rpx_wp_profile['rpx_verifiedEmail'] ) ) {
			# user exists locally, set the id in the wp profile
			$rpx_wp_profile['rpx_wp_id'] = $wpId;
			return $wpId;
		}
	}
	#either the user did not supply a verified email, the verified email link setting is disabled, or there's no current user using this email.
	return false;
}
...
function rpx_clean_locked() {
	...
	foreach ( $result as $key => $val ) {
		...
		if ( $val != NULL ) {
			if ( $val > 1 ) {
				$sqlnow = $wpdb->get_var("SELECT UTC_TIMESTAMP() FROM {$wpdb->usermeta}");
				...
				if ( strlen( $user->rpx_session ) > 1 ) {
					...
					if ( $user_aged > RPX_CLEANUP_AGE ) {
						if ( strpos( $user->user_email, $user->rpx_session) === false ) {
							//test if the email address contains the session_id
							$rpx_clean_meta = true;
						} else {
							wp_delete_user( $val );
							$count++;
...
function rpx_set_redirect($url=''){
	...
	$this_clean_url .= ($_SERVER['SERVER_PORT'] != 80) ? (':' . $_SERVER['SERVER_PORT']) : '';
	if ( !empty($url) ){
